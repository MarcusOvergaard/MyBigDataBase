...
SAMPLE_CSV="${SAMPLE_CSV:-raw_files/sample_ifs_data.csv}"
BATCH_EXTERNAL_ID="${BATCH_EXTERNAL_ID:-sample_ifs_data_csv}"

if [[ ! -f "$SAMPLE_CSV" ]]; then
    echo "IFS sample file not found: $SAMPLE_CSV" >&2
    exit 1
fi

echo "=== Loading minimal IMF IFS arbitration sample into $DB_NAME ==="

$PSQL_CMD -d "$DB_NAME" <<SQL
CREATE TEMP TABLE tmp_ifs_sample (
    country_code TEXT,
    year TEXT,
...
\\copy tmp_ifs_sample FROM '$SAMPLE_CSV' WITH (FORMAT csv, HEADER true);
...
WITH inserted_batch AS (
    ...
    SELECT
        ...
        '$BATCH_EXTERNAL_ID',
        'local://raw_files/sample_ifs_data.csv',
        jsonb_build_object('loader', 'scripts/load_ifs_sample.sh', 'sample_file', '$SAMPLE_CSV'),
        CURRENT_TIMESTAMP,
        CURRENT_TIMESTAMP,
        'queued',
        NULL
...
FROM tmp_ifs_sample s
CROSS JOIN LATERAL (
    VALUES
        ('PCPI_PC_PP_PT', 'Inflation, average consumer prices (annual %)', s.inflation_cpi_pct, 'inflation_cpi_pct'),
        ('NGDP_USD', 'Nominal GDP (current US$)', s.gdp_usd, 'gdp_usd')
...
echo "=== IMF IFS arbitration sample load complete ==="
